...
class SVC(SupervisedTabularDataModel):
    ...
    def _fit(self, data: np.ndarray, target: np.ndarray) -> "SVC":
        """_summary_
        ...
        Returns:
            SVC: the class model
        """
        self._set_init_weights(data)
        for _ in range(self.iterations):
            x_data, y_target = shuffle(data, target)  # type: ignore
...
class MaxMarginClassifier(SupervisedTabularDataModel):
    ...
    def _fit(self, dataframe: np.ndarray, target: np.ndarray) -> "MaxMarginClassifier":
        ...
        # our goal is to minimize L_d(alpha)=\sum\alpha_i -1/2\sum\sum<\alpha_iy_ix_i,\alpha_ky_kx_k>
        ...
        # and finally we will use the weights and the bias to predict the target values
        ...
        # if yi(<weights, xi> +b) > 1 the distance of xi to the hyperplan is larger than the margin.
...
def plotSvm(
    ...
    im = ax.scatter(X[:, 0], X[:, 1], X[:, 2], c=y, cmap=cmap, alpha=0.5, label=label)
...
print(model00.weights, model00.intercept)
print(model00.support_vectors)
...
print(model01.coef_, model01.intercept_)
print(model01.dual_coef_)
...
plotSvm(
    xTrain0,
    yTrain0,
    model01.support_vectors_,
    model01.coef_[0],
    model01.intercept_[0],
    label="Training",
    ax=ax,
)
fig.savefig("svc_sklearn.png")
